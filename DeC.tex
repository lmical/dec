...
We can also apply the Deferred Correction in the context of Residual Distribution and Finite Elements methods for hyperbolic balance laws to get an explicit formulation and avoid the inversion of mass matrices.
...
\item[•] Finally we will see how to apply the Deferred Correction to the context of the presented methods to get an explicit arbitrary high-order accurate formulation avoiding the inversion of mass matrices.
...
All we need to prove is now that the operators $\lopdt^1$ and $\lopdt^2$ fulfil the hypoteses required to apply the Deferred Correction. If so we can apply the method \ref{DeC} in this specific context and get
\begin{equation}
\label{DeCODE}
\lopd^1(\underline{\uvec{u}}^{(p)})=\lopd^1(\underline{\uvec{u}}^{(p-1)})-\lopd^2(\underline{\uvec{u}}^{(p-1)}) \quad p=1,2,\dots,P
\end{equation}
...
$$\uvec{0}=\sum_{K\in K_i}\sum_{\uvec {x}_j \in K} \left(\int_K \varphi_i(\uvec{x}) \varphi_j(\uvec{x}) d \uvec{x}\right)\frac{\partial}{\partial t} \uvec{c}_j(t)+\sum_{K\in K_i}\Biggl[\int_{\partial{K}} \varphi_i(\uvec{x})\uvec{F}(\sum_{j=1}^I\uvec{c}_j(t) \varphi_j(\uvec{x}))\cdot \uvec{\nu} d\sigma(\uvec{x})+$$
$$-\int_K \uvec{F}(\sum_{j=1}^I\uvec{c}_j(t) \varphi_j(\uvec{x}))\cdot \nabla_{\uvec{x}} \varphi_i(\uvec{x}) d\uvec{x}-\int_K \uvec{S}(\uvec{x},t,\sum_{j=1}^I\uvec{c}_j(t) \varphi_j(\uvec{x})) \varphi_i(\uvec{x}) d\uvec{x}+$$
\begin{equation}
\label{RDGaldiss}
+\ST(\sum_{j=1}^I\uvec{c}_j(t) \varphi_j(\uvec{x}))\Biggr]\quad t \in [0,T] \quad \forall \i=1,2,\dots,I.
...
This is the system of ODEs that we get with the Residual Distribution method and Lax-Friedrichs, SUPG and Burman node residuals.
The first term in \ref{RDGaldiss} "generates" the mass matrix $\massmatrix \in \mathbb{R}^{(I\times N)\times(I\times N)}$ which, in this case, is positive defined and thus nonsingular but unfortunately sparse\footnote{This means that we cannot rely on the efficient algorithm designed for band matrices.}.
...
$$=\sum_{K\in K_i}\beta^K_i(\uvec{u}_h)\int_K   \Biggl[  \frac{\partial}{\partial t}\uapp(\uvec{x},t)+div_{\uvec{x}} \mathbf{F}(\uapp(\uvec{x},t))-\mathbf{S}(\uvec{x},t,\uapp(\uvec{x},t))   \Biggr] d\uvec{x} \quad t \in [0,T] \quad \forall \i=1,2,\dots,I.$$
Substituiting again $\uapp(\uvec{x},t)=\sum_{j=1}^I\uvec{c}_j(t)\varphi_j(\uvec{x})$ we have
$$\uvec{0}=\sum_{K\in K_i}\beta^K_i(\sum_{j=1}^I\uvec{c}_j(t)\varphi_j(\uvec{x}))\int_K   \Biggl[  \frac{\partial}{\partial t} \left(\sum_{j=1}^I\uvec{c}_j(t)\varphi_j(\uvec{x})\right)+$$
$$+div_{\uvec{x}} \mathbf{F}(\sum_{j=1}^I\uvec{c}_j(t)\varphi_j(\uvec{x}))-\mathbf{S}(\uvec{x},t,\sum_{j=1}^I\uvec{c}_j(t)\varphi_j(\uvec{x}))   \Biggr] d\uvec{x} \quad t \in [0,T] \quad \forall \i=1,2,\dots,I$$
...
$$\uvec{0}=\sum_{K\in K_i}\beta^K_i(\sum_{j=1}^I\uvec{c}_j(t)\varphi_j(\uvec{x}))\int_K   \Biggl[  \frac{\partial}{\partial t} \left(\sum_{\uvec{x}_j \in K}\uvec{c}_j(t)\varphi_j(\uvec{x})\right)+$$
$$+div_{\uvec{x}} \mathbf{F}(\sum_{j=1}^I\uvec{c}_j(t)\varphi_j(\uvec{x}))-\mathbf{S}(\uvec{x},t,\sum_{j=1}^I\uvec{c}_j(t)\varphi_j(\uvec{x}))   \Biggr] d\uvec{x} \quad t \in [0,T] \quad \forall \i=1,2,\dots,I.$$
Thanks to basic manipulations the previous node balances become
$$\uvec{0}=\sum_{K\in K_i}\sum_{\uvec{x}_j \in K}\Biggl[\beta^K_i(\sum_{j=1}^I\uvec{c}_j(t)\varphi_j(\uvec{x}))\int_K \varphi_j(\uvec{x})d\uvec{x}\Biggr]\frac{\partial}{\partial t}\uvec{c}_j(t)+$$
$$+\sum_{K\in K_i}\beta^K_i(\sum_{j=1}^I\uvec{c}_j(t)\varphi_j(\uvec{x}))\int_K \Biggl[div_{\uvec{x}} \mathbf{F}(\sum_{j=1}^I\uvec{c}_j(t)\varphi_j(\uvec{x}))+$$
\begin{equation}
\label{RDnonvar}
-\mathbf{S}(\uvec{x},t,\sum_{j=1}^I\uvec{c}_j(t)\varphi_j(\uvec{x}))   \Biggr] d\uvec{x} \quad t \in [0,T] \quad \forall \i=1,2,\dots,I.
\end{equation}
...
As we anticipated we now project \ref{weakspace} over the finite dimensional space $(V_h)^N$ i.e. we look for $\uapp(\uvec{x},t)=\sum_{j=1}^I\uvec{c}_j(t)\varphi_j(\uvec{x}) \quad t\in [0,T]$ s.t.
...
$$
+ \int_{\Omega} \mathbf{F}(\sum_{j=1}^I\uvec{c}_i(t)\varphi_j(\uvec{x})) \cdot \nabla_{\uvec{x}} \varphi_i(\uvec{x}) d\uvec{x}=\int_{\Omega} \mathbf{S}(\uvec{x},t,\sum_{j=1}^I\uvec{c}_j(t)\varphi_j(\uvec{x}))\varphi_i(\uvec{x}) d\uvec{x}  \quad \forall i=1,2,\dots,I.
$$



This is the semidiscrete formulation of the continuous Galerkin FEM method: a system of ODEs like \ref{semidiscr} characterized by a mass matrix $\massmatrix \in \mathbb{R}^{(I \times N) \times (I \times N)}.$ 
This mass matrix is defined positive and thus nonsingular but is sparse.
