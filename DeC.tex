...
\item[•] Therefore we introduce the Discontinuous Galerkin Finite Elements approach and show how the Deferred Correction can be enforced also in this case.
...
Before presenting the Residual Distribution and the Galerkin Finite Elements methods\footnote{We will just present the methods without treating the problem of the convergence of the detected approximated solutions to the desired analytical weak ones.} and show how to apply the Deferred Correction in the context of these ones, we introduce from a general point of view the semidiscrete formulation which is a general strategy shared by several numerical methods for hyperbolic systems of balance laws\footnote{In reality the semidiscrete formulation also applies to some numerical methods for other types of equations but we will just consider hyperbolic systems of balance laws.} (among which Residual Distribution and Galerkin Finite Elements) to deal separately with the space and time discretizations. It leads to a system of ODEs commonly involving a mass matrix. We will focus on the problem of the inversion of this matrix.\\
...
\begin{observation}
Often the reference spatial domain for the numerical method is just an approximation of the analytical spatial domain $\Omega$ in which we would like to solve the initial equation. This reference domain is usually referred as "discretized". Here we assume for simplicity that the discretized domain coincides with the analytical one. Thus we use indifferently $\Omega$ for the "analytical" and "numerical" reference domains and so we will do when we will enter into the details of the Residual Distribution and the Galerkin Finite Elements methods.
\end{observation}
...
\begin{observation}
The functions $\varphi_i$ are usually but not mandatorily piecewise polynomial functions. In the case of piecewise polynomial approximations, each basis function $\varphi_i$ corresponds usually but not mandatorily to a node $\uvec{x}_i$ located somewhere in the closure of the discretized spatial domain. In these cases we have that the value of the basis function $\varphi_i$ in its associated node $\uvec{x}_i$ is not $0$ i.e. $\varphi_i(\uvec{x}_i)\neq0$. The nodes $\uvec{x}_i$ are often referred as the degrees of freedom, DoFs, of the approximated solution\footnote{As we will see, if we have continuous piecewise polynomial approximations to different basis functions are associated different nodes, while in the context of discontinuous piecewise polynomial approximations to each node are in general associated different basis functions.}. 
\end{observation}
...
\begin{observation}
The Galerkin Finite Elements and the Residual Distribution methods are based on this approach, so in both cases we get a coupled system of ODEs like the one in equation \ref{semidiscr}.
...
Some other Residual Distribution methods lead to a semidiscrete formulation very similar to the one of the (stabilized) Continuous Galerkin methods with a mass matrix not dependent on time, defined positive (thus nonsingular) but sparse. The inversion of the matrix is not cheap by a computational point of view because it is sparse and we cannot rely on the efficient algorithms designed for block diagonal matrices contrarily to the discontinuous Galerkin case. The reader could argue that this operation is done just once. This is indeed true but if we opt for a standard method requiring the inversion of the mass matrix, at each time step we will have to multiply this huge inverse for some evaluations of the vector $\uvec{H}$ (or its approximations). This results in a very big computational cost. Here the Deferred Correction comes to aid.
\end{observation}
\begin{observation}
...
\chapter[RD and continuous Galerkin FEM]{The Residual Distribution and continuous Galerkin Finite Elements methods}
We start by presenting the Residual Distribution method and then show the connection with the stabilised continuous Galerkin FEM which will be later introduced.
...
\chapter{The Deferred Correction in Residual Distribution and continuous Galerkin FEM}
