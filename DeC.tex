...
For each element $K$ of the tessellation $\tess$ we define the element residual $\elres$ which is the integral in space of the equation \ref{sys} over the element $K$
\begin{equation}
\label{elementres}
...
$$\sum_{\uvec{x}_i \in K} \noderes= \int_K \frac{\partial}{\partial t}\uvec{u}_h(\uvec{x},t) d \uvec{x}+\int_{K}div_{\uvec{x}}\uvec{F}(\uvec{u}_h(\uvec{x},t)) d\uvec{x}-\int_K \ \uvec{S}(\uvec{x},t,\uvec{u}_h(\uvec{x},t)) d\uvec{x}+ \sum_{\uvec{x}_i \in K}\ST(\uapp) \quad $$
