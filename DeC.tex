...
\chapter{Abstract} 
This work is an attempt to give a systematic introduction to the Defect Correction method which provides an iterative procedure to get arbitrary high-order integration schemes for systems of ordinary differential equations.
We can also apply the Defect Correction in the context of Residual Distribution and Finite Elements methods for hyperbolic balance laws to get an explicit formulation and avoid the inversion of mass matrices.
The structure of this script is the following:
\begin{itemize}
\item[•] We start by introducing the Defect Correction as a general method to approximate the solution of a general "tough" operator $\lopd^2$ with an arbitrary high-order precision through an iterative procedure which make use of the operator $\lopd^1$ which is a "naive" operator.
...
\item[•] In order to apply the Defect Correction to some numerical methods for solving hyperblic systems of balance laws, we present a small introduction of the analytical problem. We derive the balance laws and we introduce their weak formulation in order to overcome the limits of the classical one.
\item[•] Then we present the Discontinuous Galerkin Finite Elements and Residual Distribution approaches from a general point of view. These are the methods on which we will focus.
\item[•] Finally we will see how to apply the Defect Correction to the context of the presented methods to get an explicit arbitrary high-order accurate formulation avoiding the inversion of mass matrices.
\item[•] In the end we show how the Defect Correction method as an ODE solver can be seen as a particular Runge-Kutta scheme.
\end{itemize} 
\chapter[The Defect Correction in an abstract framework]{The Defect Correction as a general procedure in an abstract framework}
%\chapter{The Defect Correction as a general procedure in an abstract framework}
We will first introduce the Defect Correction in an abstract context.
...
\begin{theorem} \textbf{Defect Correction}\\
If 
\begin{itemize}
...
\end{observation}
Now that we have introduced and proved the Defect Correction in a general framework, the only thing that it is left to do to apply it in a more specific context is to characterize the objects needed (the operators $\lopd^1$ and $\lopd^2$, the spaces $X$ and $Y$, the norms) and verify that the three hypoteses are satisfied.
\chapter{The Defect Correction for systems of ODEs}
We will now apply the Defect Correction method to the context of the integration of systems of ordinary differential equations.
...
We are now going to put the problem in a Defect Correction framework.
...
All we need to prove is now that the operators $\lopdt^1$ and $\lopdt^2$ fulfil the hypoteses required to apply the Defect Correction. If so we can apply the method \ref{DeC} in this specific context and get
\begin{equation}
\label{DeCODE}
\lopd^1(\underline{\uvec{u}}^{(p)})=\lopd^1(\underline{\uvec{u}}^{(p-1)})-\lopd^2(\underline{\uvec{u}}^{(p-1)}) \quad p=1,2,\dots,P
\end{equation}
...
As already anticipated, at least in part, in observation \ref{optimaliterations} the Defect Correction doesn't provide an approximation of the exact solution (in the nodes $t^m$ with $m=1,2,\dots,M$) to our ODEs system but an approximation of the solution to $\lopdt^2$ which is itself a $M+1$-order approximation of the exact solution. And it would be useless to make a "too high" number of iterations to approximate it with an accuracy order higher than $M+1$. To be clearer, if we define the vector
$$\underline{\uvec{u}}^{ex}=\left(
   ...
   \end{array}
\right)$$
made by the exact solutions to our ODEs system in the nodes $t^m$ $m=1,2,\dots,M$, the Defect Correction doesn't approximate directly it but $\usoldt$ which is the solution to $\lopdt^2$ and it's a $M+1$-order accurate approximation of $\underline{\uvec{u}}^{ex}$ as we can see from the next proposition.
\begin{proposition}
\label{acc}
The vector $\underline{\uvec{u}}^{(P)}$ got from the final iteration $P$ of the Defect Correction algorithm \ref{DeCODE} is a $P$-order approximation of $\usoldt$, the solution of the operator $\lopdt^2$.
\end{proposition}
\begin{proof}
...
\end{observation}
Now we can finally prove that the hypoteses on the operators $\lopdt^1$ and $\lopdt^2$ needed to apply the Defect Correction method are fulfilled.
\section{Proof of the properties on $\lopdt^1$ and $\lopdt^2$}
Let's recall the hypoteses that are needed to apply the Defect Correction method from the abstract formulation but characterizing them to our case
\begin{itemize}
...
and prove the other two conditions. As anticipated in the observation \ref{obsnorm}, the sub-coercivity of $\lopdt^1$ will be straightforward instead the Lipschitz-continuity-like condition of $\lopdt^1-\lopdt^2$ will be less trivial (even more delicate in the application of the Defect Correction to the balance laws).
...
In order to apply the Defect Correction to some numerical methods for solving hyperbolic systems of balance laws, let's introduce the analytical problem. We would like to solve a general hyperbolic system of balance laws
...
What we have said so far can seem to be very distant from our initial goal of showing how to apply the Defect Correction to the balance laws but in the author's opinion it was fundamental in order to give a clear idea of the problem that we are trying to solve. Moreover almost all the numerical methods are designed to detect approximations of  weak (eventually entropic) solutions.
\chapter[Residual Distribution and DG]{The Residual Distribution and Discontinuous Galerkin approaches}
We will now present the Residual Distribution and Discontinuous Galerkin Finite Elements methods\footnote{We will just present the methods without treating the problem of the convergence of the detected approximated solutions to the desired analytical weak ones.} but first let's introduce in general the semidiscrete formulation and focus on the problem of the inversion of the mass matrix.
\section{Semidiscrete formulation and mass matrix}
...
\begin{observation}
The functions $\varphi_i$ are usually but not mandatorily piecewise polynomial functions. Each one of them corresponds usually to a node $\uvec{x}_i$ located somewhere in the closure of the discretized spatial domain. These nodes are often referred as the degrees of freedom, DoFs, of the approximated solution. 
\end{observation}
...
The initial $\uvec{c}(0)=\uvec{c}_0$ condition is given by the analytical initial condition on the hyperbolic system of balance laws \ref{sys} that we want to solve (projected into the finite dimensional space $(V_h)^N$).
...
\end{observation}
Let's now present more in detail the Residual Distribution and the Discontinuous Galerkin Finite Elements methods.
\section{Residual Distribution}
Again we start from our initial problem.
...
The general basis function $\varphi_i$ is associated to a node $\uvec{x}_i\in\overline{\Omega}$. These $I$ nodes associated to the $I$ basis functions are the degrees of freedom of the solution and belong in general to one or more elements of the tessellation $\tess$. Each basis function $\varphi_i$ has its support contained in the union of the elements containing the node to which it is associated.
...
\end{observation}
Now we have all the elements that we need to go into the details of the Residual Distribution approach. We will at first neglect the boundary conditions.
Basically the Residual Distri can be summarised into three steps:
\begin{itemize}
\item[i)] \textbf{Definition of the element residuals}\\
For each element $K$ of the tessellation $\tess$ we define the element residual $\uvec{\Phi}^K(t)$ which is the integral in space of the equation \ref{sys} over the element $K$
\begin{equation}
\label{elementres}
\forall K \in \tess \quad \uvec{\Phi}^K(t)=\int_K 
\left[\frac{\partial}{\partial t}\uapp(\uvec{x},t)+div_{\uvec{x}} \mathbf{F}(\uapp(\uvec{x},t))-\mathbf{S}(\uvec{x},t,\uapp(\uvec{x},t))\right] d\uvec{x}  \quad t \in [0,T];
\end{equation}
\item[ii)] \textbf{Definition of the node residuals}\\
For each element $K$ we consider the degrees of freedom belonging to it, $\uvec{x}_i \in K$, and define the node residuals $\uvec{\Phi}^K_i(t)$ such that they satisfy a specific conservation relation
$$\forall K \in \tess \quad \forall \uvec{x}_i \in K \quad \uvec{\Phi}^K_i(t) \quad t \in [0,T] \quad s.t.$$
\begin{equation}
\label{conservation}
\sum_{\uvec{x}_i \in K} \uvec{\Phi}^K_i(t)=\uvec{\Phi}^K(t) \quad \forall K \in \tess \quad \forall t  \in [0,T];
\end{equation}
\item[iii)] \textbf{Imposition of the balance at the nodes}\\
For each degree of freedom $\uvec{x}_i$ we impose an equilibrium between all the node residuals $\uvec{\Phi}^K_i(t)$ of the elements that contain that degree of freedom 
\begin{equation}
\label{balance}
\sum_{K \in K_i} \uvec{\Phi}^K_i(t)=\uvec{0} \quad t \in [0,T] \quad \forall \i=1,2,\dots,I
\end{equation}
where we recall that $K_i$ is the set of the elements of the tessellation containing the node $\uvec{x}_i$. The balance \ref{balance} represents the semidiscrete formulation of the Residual Distribution method, a system of nonlinear ODEs that we have to solve in time.
\end{itemize}
We will now give a physical interpretation of the Residual Distribution approach. Let's go back to the steps of the method and look at them under a new prespective.
\begin{itemize}
...
$$
If we integrate this quantity over each element $K$ of the tessellation at a generic time $t$ we get the integral balance
$$
\int_K \left[\frac{\partial}{\partial t}\uvec{u}(\uvec{x},t)+div_{\uvec{x}} \mathbf{F}(\uvec{u}(\uvec{x},t))-\mathbf{S}(\uvec{x},t,\uvec{u}(\uvec{x},t))\right]d\uvec{x}=\uvec{0} \quad \forall t \in \mathbb{R}^+_0.
$$
This is the meaning of the element residuals $\uvec{\Phi}^K(t)$ even if obviously to evaluate them in our numerical method we consider the approximated solution $\uapp(\uvec{x},t)=\sum_{i=1}^I\uvec{c}_i(t)\varphi_i(\uvec{x})$ in place of the exact solution $\uvec{u}$.
...
We isolate the contribution of each degree of freedom $\uvec{x}_i$ in the cell $K$ to the integral balance that we have in that cell. The node residuals $\uvec{\Phi}^K_i(t)$ are nothing more than the contributions of the nodes to the balance in the cell $K$ represented by $\uvec{\Phi}^K(t)$. This fact is expressed by the conservation relation \ref{conservation} that the node residuals must fulfil 
$$\sum_{\uvec{x}_i \in K} \uvec{\Phi}^K_i(t)=\uvec{\Phi}^K(t) \quad \forall K \in \tess \quad \forall t  \in [0,T].$$
...
\label{RD}
\end{figure}
Up to now the method is very general and also very abstract since we didn't specify how to define the node residuals $\uvec{\Phi}^K_i(t)$. The features of the particular Residual Distribution method depend on the choice of the node residuals.
We will now present some examples of suitable node residuals
...
$$\uvec{\Phi}^K_i(t)=\int_K \varphi_i(\uvec{x})\frac{\partial}{\partial t}\uvec{u}_h(\uvec{x},t) d \uvec{x}+\int_{\partial{K}} \varphi_i(\uvec{x})\uvec{F}(\uvec{u}_h(\uvec{x},t))\cdot \uvec{\nu} d\sigma(\uvec{x})-\int_K \uvec{F}(\uvec{u}_h(\uvec{x},t))\cdot \nabla_{\uvec{x}} \varphi_i(\uvec{x}) d\uvec{x}+$$
\begin{equation}
...
$$\alpha_K(\uvec{u}_h)=\sup\limits_{\substack{\uvec{x}\in f \\ f\in \partial{K}}}\rho \left( \frac{\partial \uvec{F}}{\partial \uvec{u}}(\uvec{u}_h) \nu \right)$$ 
where $\rho\left( \frac{\partial \uvec{F}}{\partial \uvec{u}}(\uvec{u}_h) \nu \right)$ is the spectral radius of the matrix $ \frac{\partial \uvec{F}}{\partial \uvec{u}}(\uvec{u}_h) \nu $ i.e. the supremum of the absolute values of its eigenvalues;
...
\item[•] \textbf{Burman}\\
$$\uvec{\Phi}^K_i(t)=\int_K \varphi_i(\uvec{x})\frac{\partial}{\partial t}\uvec{u}_h(\uvec{x},t) d \uvec{x}+\int_{\partial{K}} \varphi_i(\uvec{x})\uvec{F}(\uvec{u}_h(\uvec{x},t))\cdot \uvec{\nu} d\sigma(\uvec{x})-\int_K \uvec{F}(\uvec{u}_h(\uvec{x},t))\cdot \nabla_{\uvec{x}} \varphi_i(\uvec{x}) d\uvec{x}+$$
$$+\sum_{f\in\partial{K}}\theta h_f^2\int_f [\![ \nabla_{\uvec{x}} \uvec{u}_h(\uvec{x},t) ]\!] \cdot \nabla_{\uvec{x}} \varphi_i\vert_K(\uvec{x})  d\sigma(\uvec{x})+$$
\begin{equation}
\label{Burman}
-\int_K \uvec{S}(\uvec{x},t,\uvec{u}_h(\uvec{x},t)) \varphi_i(\uvec{x}) d\uvec{x} \quad t\in [0,T]
\end{equation}
where $f$ is a generic $D-1$-dimensional face of the generic element $K$ of the tessellation, $\theta\in \mathbb{R^+}$ is a positive parameter, $h_f$ is a constant depending on the face $f$, $[\![ \cdot ]\!]$ represents the jump of the argument across $f$ i.e.
...
\begin{equation}
\label{nonvar}
\uvec{\Phi}^K_i(t)=\beta^K_i(\uvec{u}_h) \uvec{\Phi}^K(t) \quad t\in [0,T]
\end{equation}
where $\beta^K_i(\uvec{u}_h)$ are some coefficients depending on the approximated solution verifying 
\begin{equation}
\label{coefficients}
...
$$\sum_{\uvec{x}_i \in K}\uvec{\Phi}^K_i(t)=\sum_{\uvec{x}_i \in K}\beta^K_i(\uvec{u}_h)\uvec{\Phi}^K(t)=\left(\sum_{\uvec{x}_i \in K}\beta^K_i(\uvec{u}_h)\right)\uvec{\Phi}^K(t)=\uvec{\Phi}^K(t).$$
...
\item[•] \textbf{Burman}
$$\ST(\uapp)=\sum_{f\in\partial{K}}\theta h_f^2\int_f [\![ \nabla_{\uvec{x}} \uvec{u}_h(\uvec{x},t) ]\!]  \cdot \nabla_{\uvec{x}} \varphi_i\vert_K(\uvec{x}) d\sigma(\uvec{x})\quad t\in [0,T].$$

\end{itemize}
If we make now the sum of these nodal residuals associated to the nodes of a cell $K$ we get
$$\sum_{\uvec{x}_i \in K} \uvec{\Phi}^K_i(t)=\sum_{\uvec{x}_i \in K}\Biggl[ \int_K \varphi_i(\uvec{x})\frac{\partial}{\partial t}\uvec{u}_h(\uvec{x},t) d \uvec{x}+\int_{\partial{K}} \varphi_i(\uvec{x})\uvec{F}(\uvec{u}_h(\uvec{x},t))\cdot \uvec{\nu} d\sigma(\uvec{x})+$$
...
$$\sum_{\uvec{x}_i \in K} \uvec{\Phi}^K_i(t)= \int_K \left(\sum_{\uvec{x}_i \in K}\varphi_i(\uvec{x})\right)\frac{\partial}{\partial t}\uvec{u}_h(\uvec{x},t) d \uvec{x}+\int_{\partial{K}}\left(\sum_{\uvec{x}_i \in K}\varphi_i(\uvec{x})\right)\uvec{F}(\uvec{u}_h(\uvec{x},t))\cdot \uvec{\nu} d\sigma(\uvec{x})+$$ 
...
\end{equation}
Thus we get
$$\sum_{\uvec{x}_i \in K} \uvec{\Phi}^K_i(t)= \int_K \frac{\partial}{\partial t}\uvec{u}_h(\uvec{x},t) d \uvec{x}+\int_{\partial{K}}\uvec{F}(\uvec{u}_h(\uvec{x},t))\cdot \uvec{\nu} d\sigma(\uvec{x})-\int_K \ \uvec{S}(\uvec{x},t,\uvec{u}_h(\uvec{x},t)) d\uvec{x}+ \sum_{\uvec{x}_i \in K}\ST(\uapp)$$
and applying the Gauss theorem to the second term of the right hand side we have
$$\sum_{\uvec{x}_i \in K} \uvec{\Phi}^K_i(t)= \int_K \frac{\partial}{\partial t}\uvec{u}_h(\uvec{x},t) d \uvec{x}+\int_{K}div_{\uvec{x}}\uvec{F}(\uvec{u}_h(\uvec{x},t)) d\uvec{x}-\int_K \ \uvec{S}(\uvec{x},t,\uvec{u}_h(\uvec{x},t)) d\uvec{x}+ \sum_{\uvec{x}_i \in K}\ST(\uapp) \quad $$
which, recalling the definition \ref{elementres} of the element residuals, reads
\begin{equation}
\label{intermediate}
\sum_{\uvec{x}_i \in K}\uvec{\Phi}^K_i(t)=\uvec{\Phi}^K(t)+\sum_{\uvec{x}_i \in K}\ST(\uapp) \quad t\in [0,T].
\end{equation}
...
\item[•] \textbf{Burman}
$$\sum_{\uvec{x}_i \in K}\ST(\uapp)=\sum_{\uvec{x}_i \in K}\sum_{f\in\partial{K}}\theta h_f^2\int_f [\![ \nabla_{\uvec{x}} \uvec{u}_h(\uvec{x},t) ]\!] \cdot \nabla_{\uvec{x}} \varphi_i\vert_K(\uvec{x}) d\sigma(\uvec{x})=$$
...
We will talk more in detail about the Galerkin finite elements method, or FEM, later in the context of the Dicontinuous Galerkin schemes, but it's better to introduce some concepts now in order to have a clearer understanding of the meaning of the three mentioned node residuals.
...
$$V_h=\lbrace g \in C^0(\overline{\Omega}) \quad s.t. \quad g\vert_K \in \mathbb{P}_M \quad \forall K \in \tess\rbrace.$$
Let's remind that each function $\varphi_i$ is associated to a node $\uvec{x}_i \in \overline{\Omega}$ and has support in the union of the elements of the tessellation containing that node.
As we anticipated we now project \ref{weakspace} over the finite dimensional space $(V_h)^N$ i.e. we look for $\uapp(\uvec{x},t)=\sum_{i=1}^I\uvec{c}_i(t)\varphi_i(\uvec{x}) \quad t\in [0,T]$ s.t.
...
$$\int_{\Omega} \varphi_r(\uvec{x}) \frac{\partial}{\partial t}\uapp(\uvec{x},t)  d\uvec{x}+ \int_{\partial \Omega} \varphi_r(\uvec{x})\mathbf{F}(\uapp(\uvec{x},t))\cdot\uvec{\nu}d\sigma(\uvec{x})+$$
$$
- \int_{\Omega} \mathbf{F}(\uapp(\uvec{x},t)) \cdot \nabla_{\uvec{x}} \varphi_r(\uvec{x}) d\uvec{x}=\int_{\Omega} \mathbf{S}(\uvec{x},t,\uapp(\uvec{x},t))\varphi_r(\uvec{x},t) d\uvec{x}  \quad \forall r=1,2,\dots,I.
$$
If we now substitute $\uapp(\uvec{x},t)=\sum_{i=1}^I\uvec{c}_i(t)\varphi_i(\uvec{x})$ in the previous equation we get
$$\int_{\Omega} \varphi_r(\uvec{x}) \frac{\partial}{\partial t}\left( \sum_{i=1}^I\uvec{c}_i(t)\varphi_i(\uvec{x})\right)  d\uvec{x}+ \int_{\partial \Omega} \varphi_r(\uvec{x})\mathbf{F}(\sum_{i=1}^I\uvec{c}_i(t)\varphi_i(\uvec{x}))\cdot\uvec{\nu}d\sigma(\uvec{x})+$$
$$
- \int_{\Omega} \mathbf{F}(\sum_{i=1}^I\uvec{c}_i(t)\varphi_i(\uvec{x})) \cdot \nabla_{\uvec{x}} \varphi_r(\uvec{x}) d\uvec{x}=\int_{\Omega} \mathbf{S}(\uvec{x},t,\sum_{i=1}^I\uvec{c}_i(t)\varphi_i(\uvec{x}))\varphi_r(\uvec{x},t) d\uvec{x}  \quad \forall r=1,2,\dots,I.
$$
This is the semidiscrete formulation of the method a system of ODEs characterized by a mass-matrix
\end{observation}

  
%Derivation of the first three 
%Galerkin with stabilization term

%Lax Friedrichs SUPG only first order accurate
...
\chapter[The Defect Correction in balance laws]{The Defect Correction in balance laws as an explicit time integrator}
